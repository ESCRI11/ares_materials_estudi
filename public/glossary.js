...
const GLOSSARY = {
	...
	powershell: "The shell that comes with Windows: the program inside the terminal window that reads and runs your commands. Its prompt starts with PS. Capable, but it is not Unix, which is why this site installs WSL2 and uses bash instead.",
	...
	flag: "A switch in a terminal command that changes how it behaves, written with a dash — the -l in ls -l or the -a in ls -a. Several can be combined, as in ls -la.",
	...
	ls: "A command that lists the files and folders in the working directory. Add -l for sizes and dates, -a to include hidden files, or -la for both.",
	...
	winget: "The package manager built into Windows, run from PowerShell. With WSL2 in place, use it for Windows applications with a window — a browser, Figma, VS Code — and use apt inside Ubuntu for command-line tools.",
	...
	head: "Two meanings. In HTML, the part of a document holding things the reader does not see directly \u2014 the title, the character set, the link to your stylesheet. In git, HEAD is where you are standing right now: the commit your next commit will build on, which is why it labels your side of a merge conflict.",
	body: "Two meanings. In HTML, the part of a document that holds everything the reader sees. In CAD, one lump of solid geometry \u2014 extrude something and a body appears. Bodies are shapes with no names and no relationship to each other; components are the named parts of an assembly.",
	...
	callback: "A function you hand to something else so that it can run it later, rather than calling it yourself. The function passed to addEventListener, map or filter is one.",
	...
	node: "Two meanings. In a page or an audio chain it is one piece of a connected structure: a single element in the page tree, or a single block \u2014 a tone, a volume control \u2014 in a chain of audio processing. Written with a capital N it is Node, the program that runs JavaScript outside the browser, on your own machine.",
	...
	kerf: "The width of material a cutting beam or blade removes as it travels, roughly a tenth to three tenths of a millimetre on a laser. Because the beam is centred on your line, holes come out oversized and tabs come out undersized, so an exact-looking fit ends up loose by two kerfs \u2014 one from each part.",
	...
	overhang: "Part of a 3D model that sticks out over empty space, with nothing beneath it for the next layer to rest on. Up to about 45 degrees away from vertical it prints cleanly; lean it further out than that, towards a flat ceiling, and it droops.",
	...
	distribution: "Two meanings. In Linux it is one packaged version of the system — Ubuntu, Debian, Fedora — with the same core and different tools and defaults on top. With a language model it is the set of scores across all the possible next tokens, saying how likely each one is; the model produces one at every step, and temperature reshapes it before a token is drawn.",
	...
	delimiter: "The character that separates one value from the next in a text file. A comma in most CSV files, but a semicolon wherever the exporting machine's regional settings say so \u2014 common across Europe, where a comma is often the decimal mark. Swiss numbers use a point for the decimal and an apostrophe for thousands, so the reason there is regional settings rather than the decimal comma.",
	...
	multimodal: "Two meanings. In interaction design, using more than one modality in a single experience \u2014 either together (point and speak) or in sequence (start by voice, confirm on a screen). Of a language model, one that accepts more than text in a request: an image, and sometimes audio, alongside the words.",
	...
	"hidden file": "A file whose name starts with a dot, like .gitignore. It is not secret — the convention exists to keep configuration files out of your way — and ls -a reveals them.",
	...
	// added from 10
	wsl: "Windows Subsystem for Linux: the Microsoft feature that runs a real Linux system alongside Windows on the same machine, giving you the Unix shell every tutorial and lecturer assumes.",
	wsl2: "The second version of the Windows Subsystem for Linux, and the one everybody means. It runs a genuine Linux kernel next to Windows, so Linux commands are not imitated or translated — they are the real thing.",
	ubuntu: "The most widely used Linux distribution, and the one WSL installs by default. Choose it not because it is technically better but because when you search an error message at midnight, the person who answered was using it.",
	linux: "A free operating system, the one nearly every web server in the world runs. It comes in distributions rather than as a single product, and on Windows you reach it through WSL2.",
	"linux distribution": "Linux packaged up with a particular set of tools, defaults and a name — Ubuntu, Debian, Fedora. Same core underneath, different opinions on top.",
	distro: "Short for distribution, in the Linux sense: one packaged version of Linux, such as Ubuntu.",
	unix: "The family of operating systems macOS and Linux both descend from. They share a shell vocabulary — ls, cd, pwd, sudo — that has barely changed in fifty years, and effectively all programming documentation is written in it.",
	kernel: "The core of an operating system: the part that talks to the hardware and hands out memory, files and processing time to everything else. WSL2 runs a real Linux one alongside Windows.",
	bash: "The standard Linux shell, and the one running inside Ubuntu on WSL2. Close enough to macOS's zsh that commands written for one work in the other.",
	apt: "Ubuntu's package manager, used from inside WSL2 on Windows. sudo apt update refreshes the catalogue, sudo apt install adds something, sudo apt upgrade installs newer versions of what you already have.",
	"apt update": "Downloads a fresh copy of the package catalogue — the list of what exists and at which version. It installs nothing. The command that actually installs newer versions is apt upgrade, and it can only fetch what the catalogue knows about.",
	"/mnt/c": "Where your Windows C: drive appears from inside WSL2. mnt is short for mounted, the Unix word for a disk attached to the tree. Reach files here, but keep your own project folders in the Linux home instead — working across the boundary is slow.",
	mount: "To attach a disk to the folder tree so its contents can be reached by path. Your Windows drives are mounted under /mnt inside WSL2.",
	"virtual machine": "A whole second computer pretending to exist inside your real one, with its own memory allowance and boot sequence. WSL2 uses this machinery underneath but hides all of it, so there is nothing for you to manage.",
	"windows terminal": "The terminal application on Windows, which holds several shells in tabs — PowerShell in one, Ubuntu in another. Preinstalled on Windows 11, free from the Microsoft Store on Windows 10.",
	"wsl extension": "The VS Code add-on published by Microsoft that keeps the editor window on Windows while everything behind it — the file tree, search, the terminal, git — runs inside Ubuntu, next to your files. The bottom-left corner reads WSL: Ubuntu when it is working.",
	bios: "The small settings program built into a computer itself, which runs before Windows starts. WSL2 needs a processor feature called virtualisation, which some laptops ship with switched off in here.",
	virtualisation: "A processor feature that lets one machine run another operating system alongside its own. WSL2 needs it, and Task Manager's Performance tab tells you whether your laptop has it enabled.",
	...
	"permission mode": "How much an AI coding tool asks before acting. Default asks before every change, accept-edits stops asking for file edits, plan mode never changes anything, and auto stops asking altogether and has a second model check each action instead of you.",
	...
	// added while fixing the review findings
	amend: "Replacing the last commit rather than adding another, with git commit --amend. Useful for a bad message or a forgotten file, and only safe before that commit has been pushed.",
	"array method": "One of the functions built into every array \u2014 map, filter, reduce, forEach and a few dozen more. You hand each of them a function, and it runs that function once for every item.",
	assembly: "A model made of several components positioned relative to each other, rather than one lump of geometry. Joints, exploded views and parts lists all need one.",
	"auto mode": "The Claude Code permission mode in which nothing routine stops to ask you \u2014 edits, commands and network requests all run, and a separate model checks each action first and blocks a list of dangerous ones. It reduces prompts; it does not guarantee safety, and it removes the diff you would otherwise have read.",
	"binary file": "A file that is not plain text \u2014 an image, a video, a PSD, a compiled program. Git can record one and tell you it changed, but it cannot show you what changed inside it.",
	bodies: "In CAD, the lumps of solid geometry in a design. A body is just a shape: to position one part against another, to explode an assembly or to list parts on a drawing, the bodies have to be turned into named components first.",
	cache: "A local store of files a page has already fetched \u2014 a stylesheet, an image \u2014 so they do not have to be fetched again. Helpful, until it hands you yesterday's version of a file you just changed.",
	caching: "Keeping a local copy of a file a page has already fetched, so it does not have to be fetched again. A hard reload forces fresh copies.",
	"conflict marker": "The lines git writes into a file when a merge conflict stops it: <<<<<<< HEAD above one version, ======= between them, >>>>>>> above the other. They are ordinary text, and resolving the conflict means deleting them along with the version you do not want.",
	continue: "In a loop, the keyword that abandons the current turn and jumps straight to the next value. Everything below it in the loop body is skipped for that one item.",
	"ds_store": "A small invisible file macOS drops into every folder you open in Finder, recording icon positions. It is meaningless to anyone else, which is why .DS_Store belongs in every .gitignore.",
	echo: "A terminal command that prints whatever you give it. Paired with > it is the quickest way to write one line into a new file.",
	env: "A file named .env, holding keys and passwords a program needs. It is listed in .gitignore from the first commit, because a key committed once stays in the history.",
	filter: "Two meanings. In JavaScript, an array method that builds a new list of only the items your function answered true for. More loosely, narrowing a set of data or findings down to the ones that matter.",
	focus: "The state of the element currently receiving keyboard input, after a Tab press or a click into a field, selected in CSS with `:focus`. Browsers draw a ring around it; restyle that ring, never remove it.",
	foreach: "An array method that runs your function once for each item and hands nothing back. The same job as a for...of loop, written as a method.",
	"function calling": "Another name for tool use: describing functions to a model so it can ask for one to be called, with your code deciding whether to run it.",
	generative: "Made by a set of rules that leaves some room in it, so running the same program twice gives two different results. The thing you design is the system rather than the single outcome.",
	hairline: "A line with no thickness of its own, which is how vector artwork says cut rather than engrave to a laser cutter. Software without a hairline setting usually wants 0.001 inch, about 0.025 mm. The lab decides which convention its machine reads.",
	"hard reload": "A reload that ignores the cache and fetches every linked file again: Ctrl+Shift+R on Windows and Linux, Cmd+Shift+R on macOS, or Shift and the reload button. What to try when a saved change refuses to appear.",
	hover: "The state of an element while the pointer is over it, selected in CSS with `:hover`. Touch screens have no pointer, so nothing important should be reachable only this way.",
	"image input": "Sending a model a picture alongside the text of a request \u2014 a photograph, a screenshot, a scan, a frame from a camera \u2014 and getting words back. It turns a camera into a sensor that reports in language, and it costs far more tokens than a sentence does.",
	lts: "Long Term Support \u2014 the release of a tool that is maintained for years rather than months. The version to install when a download page offers you a choice.",
	map: "Two meanings. In JavaScript, an array method that builds a new list by running your function on every item \u2014 same length, one new value each, original untouched. In design research, a diagram of an experience over time: a journey map, a service blueprint.",
	"math.random": "The function that gives back a decimal number that is at least 0 and always less than 1, different every time. Multiply it to stretch that range and add to shift it.",
	mdn: "MDN Web Docs, the reference for HTML, CSS and JavaScript, maintained by Mozilla with the people who build browsers. Search for `mdn` plus the thing you are stuck on; the page opens with what it is, then runnable examples.",
	"mdn web docs": "The reference for HTML, CSS and JavaScript, maintained by Mozilla with the people who build browsers. Free, no account, and the source to trust when two sites disagree.",
	method: "A function that belongs to a value and is written after a dot, like list.push(4) or name.trim(). Same idea as any other function; the dot says what it acts on.",
	nano: "A plain text editor that runs inside the terminal, and the one Ubuntu opens by default. Its commands are listed along the bottom of the screen: Ctrl+X leaves, then Y to save.",
	"node_modules": "The folder a JavaScript project downloads its libraries into. It is large, it is rebuilt from a list whenever it is needed, and it should never be committed.",
	npm: "The package manager that arrives with Node.js. It downloads JavaScript libraries into your project, in the same way apt and Homebrew install programs on your machine.",
	pager: "The viewer git and other tools send long output to, showing one screen at a time and waiting. It looks like a frozen terminal and is not \u2014 press q to leave.",
	"process.env": "How a program running in Node reads the environment variables of the terminal it was started from. The standard way to get an API key into your code without writing it in a file.",
	"pseudo-class": "A selector for an element in a particular state, written with a colon after it: `a:hover`, `input:focus`. The state is not a class you typed into the HTML \u2014 the browser applies it and takes it away again.",
	reduce: "An array method that boils a whole list down to a single value, such as a total. Its function receives the result so far and the current item, and the second argument to reduce is where the result starts.",
	restore: "The git command that throws work away on purpose. git restore <file> returns a file to how it was in the last commit; git restore --staged <file> takes it back out of the staging area, keeping the edit.",
	"rewind menu": "The list of your own messages that Esc twice opens in Claude Code, from which you can restore the conversation, the code, or both, to an earlier point. It only covers edits the tool made with its own file-editing tools \u2014 not what a shell command did, and not what you changed yourself.",
	schema: "Two meanings, closely related. In a database it is the structure the data must have: which tables exist, which columns each holds, and what type each column is \u2014 written down before any data goes in. Sent to a language model, it is the same idea used to constrain a reply, stating which fields and which types the answer must come back in.",
	seed: "A starting number given to a random generator so that it produces the same sequence every time. Math.random cannot be seeded, which is why a result you like cannot be asked for again.",
	"structured output": "Requiring a model's reply to match a shape you defined \u2014 which fields, which types, which allowed values \u2014 with the provider constraining generation so the reply is valid by construction rather than because you asked politely. It removes parsing bugs, not hallucination: the shape is enforced, the contents are not.",
	"tool use": "Sending a model a list of functions your code can run, so that instead of answering it may reply asking for one of them to be called with particular arguments. The model never runs anything itself \u2014 it emits a name and some arguments, and your code decides whether to obey. Also called function calling.",
	tracked: "A file git has been told to record, so it appears in commits and git status reports when it changes. The opposite of untracked, and the reason .gitignore has no effect on a file you already committed.",
	vim: "A text editor that runs inside the terminal, and the one macOS opens by default. It starts in a mode where keys are commands: press i to type, Esc to stop, then :wq and Enter to save and leave, or :q! to leave without saving.",

	// added with the documentation and python/sql sections
	activate: "To switch a terminal window over to a project's virtual environment, with source .venv/bin/activate. Your prompt gains a (.venv) label, and python and pip now mean the ones inside that folder. It applies to that window only.",
	"b-roll": "Supporting footage you cut away to when a main shot is too short or too dull: slow moves across an object, close-ups of a detail. Cheap to shoot and the thing an edit always runs out of.",
	"bill of materials": "A list of every part in a project with its quantity and value, so somebody could order the parts without measuring your prototype.",
	bom: "Short for bill of materials.",
	"build log": "A short note written at the end of each working session: what you tried, what happened, what you changed. Raw material for the final documentation, kept while the material still exists.",
	column: "One field in a database table, declared once with a name and a type. Unlike a spreadsheet column, nobody can put a word in it halfway down without the database noticing.",
	database: "Data stored in a structured way that software itself enforces, rather than in a file whose shape depends on whoever last typed in it. Several programs can read and write it at once, and it answers questions without you writing the loop.",
	dataframe: "A whole table held in memory by pandas, which knows its own column names and knows which columns hold numbers. The unit everything in pandas acts on.",
	deliverable: "A named thing you have to hand in. The syllabus lists them explicitly, so an unlisted extra earns nothing and a missing one costs the whole module.",
	dictreader: "The reader in Python's csv module that hands you one dictionary per line of the file, keyed by the names in the header row \u2014 so you write row[\"city\"] rather than counting commas.",
	documentation: "The record of a project that is submitted alongside it: a written description, photographs or screenshots, a video, the source files and the schematics. In two semester-1 modules it is assessed, not optional.",
	ects: "European Credit Transfer System: the unit European universities measure a degree in. Roughly 25 to 30 hours of work, including the hours you put in outside class.",
	"externally-managed-environment": "The error a recent Ubuntu or Homebrew Python gives when you pip install outside a virtual environment. It is a refusal, not a permissions problem: that Python belongs to the operating system, and replacing a library in it can break system tools. Make a virtual environment rather than reaching for sudo.",
	"foreign key": "A column holding another table's primary key. It is the link between two tables, and the reason a fact like a venue's name can be stored once and referred to everywhere.",
	fritzing: "A program for drawing circuits, which shows the same one as a breadboard, as a schematic and as a circuit board. Switching between the first two views is how the pictorial and symbolic versions connect.",
	"group by": "The part of a SQL query that gathers rows sharing a value and produces one output row per group, so COUNT and SUM report per venue or per city instead of once for everything. The same job as building a dictionary of running totals by hand.",
	join: "The SQL operation that pairs rows from two tables using a column they have in common, so a query can return the venue's name alongside the visit's date. It is the reason splitting data across tables costs nothing to read back.",
	kicad: "Free, professional software for designing circuits and circuit boards. Far more tool than a course project needs, and a real skill if you want it.",
	pandas: "The main Python library for working with tables of data. Not part of the standard library, so it has to be installed; worth it once the operations pile up, and more than you need for a few thousand rows and one question.",
	"pin table": "A table listing every connection in a circuit by the printed pin labels at each end. The cheapest way to remove the ambiguity a photograph of a breadboard leaves behind.",
	pip: "Python's package manager: the command that downloads libraries into your project, the way npm does for JavaScript and apt does for Ubuntu. Run it inside a virtual environment, never against the Python that came with your system.",
	"primary key": "The column whose value identifies a row and never repeats, usually a plain counter called id. It is how you refer to one exact row, and its uniqueness is enforced rather than hoped for.",
	query: "One question put to a database, written in SQL. It takes a table and gives back a table \u2014 even a single number comes back as one row and one column.",
	relational: "Describing a database built out of tables that refer to one another by key, rather than one large table with everything in it. Nearly every database you will meet is this kind.",
	"requirements.txt": "A plain list of the packages a Python project needs, produced with pip freeze and installed with pip install -r. The equivalent of the dependency list in a JavaScript project.",
	row: "One record in a database table: one visit, one venue, one person. The same idea as a row in a tidy spreadsheet, with the shape enforced.",
	sql: "Structured Query Language, said as 'sequel' or as three letters: the language for asking questions of a database. You describe the rows you want and the database works out how to find them, so there is no loop to write.",
	sqlite: "A database that lives in a single ordinary file, with no server, no account and nothing running. It is in every phone and browser, Python can open one with no installation at all, and it is the right choice whenever the data belongs to one project.",
	"standard library": "The modules that arrive with Python and need no installing \u2014 csv, json, datetime, random, statistics, pathlib and a few hundred more. import one and it is there.",
	table: "In a database, the store for one kind of thing \u2014 visits in one, venues in another. One row per record, one column per field, and the columns are declared with names and types before any data goes in.",
	venv: "The Python module that creates a virtual environment, used as python3 -m venv .venv. Also the usual name of the folder it makes.",
	"virtual environment": "A folder holding its own Python and its own set of installed packages, belonging to one project. It is what lets pip install anything you like without touching the Python your operating system depends on.",
	"wiring diagram": "A drawing that shows the parts of a circuit where they physically sit, with the wires between them. Easier to follow than a schematic and worse for understanding, because the layout hides the circuit.",
};
