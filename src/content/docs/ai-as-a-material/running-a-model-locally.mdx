...
**The three capabilities from the last lesson, unevenly.** Structured output, tool use and image
input are supported by some local models and not others, and a small model that technically
supports them is markedly worse at them: it invents field names, calls the wrong tool, or calls
none when it should. Check what a specific model claims before you design around it, and expect
...
Both of these can also run as a **local server** on your machine, which means the code from
[calling a model](/ares_materials_estudi/ai-as-a-material/calling-a-model/) works nearly
