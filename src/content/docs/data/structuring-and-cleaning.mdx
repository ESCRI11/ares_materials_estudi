...
Two pieces of JavaScript in there are new.

`continue` abandons the current turn of the loop and jumps straight to the next value. The
counting line below it never runs for a spelling the lookup did not recognise.

`tally[name] = (tally[name] || 0) + 1` is the counting idiom. Between two values rather than
two comparisons, `||` is not the boolean *or* from
[conditionals](/ares_materials_estudi/javascript/conditionals/): it hands back the left side
if there is anything in it, and the right side otherwise. `tally[name]` is `undefined` the
first time a city appears, so you get `0 + 1`, and after that the running total plus one.

Add `"ZH"` to the raw list and press Run — an unrecognised value announces itself rather than
vanishing, which is the behaviour you want.
	...
	questions={[
		{
			...
			options: [
				'Your sample really did skew towards much older participants',
				'999 stands for a missing value and is averaged in as a real age',
				'The mean is the wrong measure here — use the median instead',
			],
			answer: 1,
			explanation:
				'Stand-ins like 999, -1 and N/A are more dangerous than blanks, because they survive every check that looks for emptiness. Find them, convert them to genuine missing values, and decide explicitly what each calculation does with them. Switching to the median would only hide the 999s, which are still in your dataset.',
		},
		{
			...
			options: [
				'Delete the 300,000 row, since one studio should not drive the chart',
				'Look at that record, report the median with the mean, and log it',
				'Report only the mean, because it is the measure that uses every value',
			],
			answer: 1,
			explanation:
		...
		},
		{
			...
			options: [
				'So there is a backup if the cleaned file is lost or overwritten',
				'Because manual edits to the only copy are unrepeatable and invisible',
				'Because the raw file is the one you hand in with the project',
			],
			answer: 1,
			explanation:
