	...
	questions={[
		{
			...
			options: [
				'A phone app showing live status for every machine in the room',
				'A light on the machine, visible from across the room',
				'A voice assistant that answers when a technician asks it',
			],
			answer: 1,
			explanation:
		...
		},
		{
			...
			options: [
				'Before drafting, since that is where the agent starts acting for you',
				'Before sending, the irreversible and publicly visible step',
				'Nowhere, once the drafts test as accurate enough to trust',
			],
			answer: 1,
			explanation:
		...
		},
		{
			...
			options: [
				'Eight participants is too small a sample to say anything at all',
				'One run each cannot characterise behaviour that varies run to run',
