...
Physical artefacts are gone in a week; the transcription is what you actually reuse.
	...
	questions={[
		{
			...
			options: [
				'Seven of ten participants abandoned the form at the address step',
				'Participants abandon the address step because it asks for a permanent address, and most of them move every year',
				'Participants said the form felt long and asked for too much',
			],
			answer: 1,
			explanation:
				'The first is a finding — a counted pattern with no explanation. The third is a summary of reported feeling, and the vaguest of the three. Only the second explains the behaviour, and only it changes what you would do: the fix is not a shorter form, it is a form that does not assume address stability.',
		},
		{
			q: 'During affinity mapping one note refuses to fit any cluster. What is the right move?',
			options: [
				'Put it in the nearest cluster so the wall stays tidy',
				'Discard it as an outlier',
				'Leave it in a separate pile and keep it in the write-up',
			],
			answer: 2,
			explanation:
