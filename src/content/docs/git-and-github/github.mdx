...
was switched off in 2021 and every old tutorial that says otherwise is wrong. Two things do
work, and both behave the same inside Ubuntu as they do on a Mac:

- **A personal access token** — a long generated string you paste where git asks for a
  password. You make one in GitHub's settings, under developer settings. It works
  immediately, and git asks for it again on the next push unless something on your machine
  has stored it.
- **An SSH key** — a pair of files on your machine that proves your identity without typing
  anything. More setup once, nothing to type ever again. If you plan to use git for the whole
  master, this is the one worth doing.

:::note[If a browser window opens, take it]
On a Mac, and on Windows installs that came with git's own bundle, a helper program
sometimes pops up a browser sign-in the first time and remembers the answer afterwards.
Inside Ubuntu on WSL2 that does not happen by default: you get a plain `Username` and
`Password` prompt in the terminal, and the "password" it wants is the token. That is the
prompt working correctly, not a broken install.
:::
...
[docs.github.com/en/authentication](https://docs.github.com/en/authentication) covers both,
...
## What it looks like once it is up there

Here is a real repository — this site's own — a few minutes after a push:

![A GitHub repository page: a branch selector reading main, a commit count, and a list of folders and files. Each row shows the file name, the message from the last commit that touched it, and how long ago that was.](../../../assets/screenshots/github-repo-page.png)

Read the middle column. Every file carries the message from the last commit that changed
it, which is why the advice about writing a useful message is not housekeeping — this is
the page where those messages get read, by you in three months and by anyone else looking
at your work.
...
- [ ] I got past authentication with a token or an SSH key, and I know my account password is not it
