...
<Quiz
	...
	questions={[
		...
		},
		{
			...
			options: [
				'A for loop would run every repetition before the browser could draw anything, freezing the page',
				'for loops cannot contain drawing commands',
				'requestAnimationFrame is faster arithmetic',
			],
			answer: 0,
			explanation:
		...
		},
		{
			...
			options: [
				'Moves x at a constant ten pixels per frame',
				'Moves x a tenth of the remaining distance each frame, so it decelerates as it arrives',
				'Sets x to a tenth of targetX',
			],
			answer: 1,
			explanation:
