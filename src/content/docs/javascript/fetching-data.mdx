	...
	questions={[
		{
			...
			options: [
				'Pauses the whole browser until the data arrives',
				'Waits for the promised value inside an async function, then continues with the real value',
				'Repeats the request until it succeeds',
			],
			answer: 1,
			explanation:
		...
		},
		{
			...
			options: [
				'404 responses are silently retried',
				'A 404 is a successful reply whose content is an error, so fetch does not throw — you check response.ok',
				'catch only works with JSON.parse',
			],
			answer: 1,
			explanation:
		...
		},
		{
			...
			options: [
				'Because what arrives over the network is text, and parse turns that text into values',
				'Because JSON uses different brackets',
