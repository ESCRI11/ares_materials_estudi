...
becomes a tree — `body` holds an `h1` and a `ul`, the `ul` holds two `li`. Each of those is an
...
There is a second property, `innerHTML`, that accepts tags rather than plain text. It is also
how pages get attacked: text typed by a visitor and pushed into `innerHTML` can carry a script
with it. Use `textContent` unless you specifically need to insert markup.
:::
...
<Quiz
	...
	questions={[
		...
		},
		{
			...
			options: [
				'createElement is not a real function',
				'The element exists but was never appended to anything on the page',
				'List items only work inside a table',
			],
			answer: 1,
			explanation:
