	...
	questions={[
		{
			...
			options: [
				'Because loops are faster than running code once',
				'Because there is no operating system to hand control back to when the program finishes',
				'Because MicroPython requires it',
			],
			answer: 1,
			explanation:
		...
		},
		{
			...
			options: [
				'Digital is faster; analog is slower',
				'Digital has two states, high or low; analog is a continuous range of voltages between 0 V and the supply',
				'Digital is for output and analog is for input',
			],
			answer: 1,
			explanation:
		...
		},
		{
			...
			options: [
				'The pin voltage is too high for a motor',
				'A pin can only supply a small current — a motor needs far more than it can give',
