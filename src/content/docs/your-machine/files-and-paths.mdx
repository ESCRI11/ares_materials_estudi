...
everything else you own live inside it. It has a shorthand, `~` (tilde), which every system
here understands in most places. So `~/Downloads` means `/Users/ares/Downloads` on a Mac
...
where your project files will live, and the WSL2 lesson explains why.
...
<Quiz
	questions={[
		...
		},
		{
			...
			options: [
				'/Users/ares/Desktop/brief.pdf',
				'/mnt/c/Users/ares/Desktop/brief.pdf',
				'~/Desktop/brief.pdf',
			],
			answer: 1,
			explanation:
				'Two rules, no more: the drive letter becomes /mnt/ plus that letter in lowercase, and every backslash becomes a forward slash. It is one disk described twice, not two copies of the file. The tilde is something else again — it means the Linux home, which is a separate place from the Windows one.',
		},
	]}
/>
