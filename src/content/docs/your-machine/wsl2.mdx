...
Newer versions of Windows spell the same address `\\wsl.localhost\Ubuntu\home\ares`, and
both forms still work. Use whichever your File Explorer offers when you go looking; they
...
- [ ] I found my Linux home from Windows by typing `\\wsl$\Ubuntu` or `\\wsl.localhost\Ubuntu` into File Explorer
	...
	questions={[
		{
			...
			options: [
				...
				'A real Linux kernel running beside Windows on one machine',
		...
		},
		{
			...
			options: [
				'Your Windows sign-in password, reused inside Ubuntu',
				'Your Microsoft account password, since Microsoft ships WSL',
				'A new one you invent now, used only inside Linux',
			],
			answer: 2,
			explanation:
